...
class Args:
    ...
    def parse(self, argv: Sequence[str], meta: dict[str, Any]) -> tuple[dict[str, Any], CustomArgumentParser, list[str]]:
        ...
        parser.add_argument('-radarr-add', '--radarr-add', action='store_true', required=False, help="Use queue/qBittorrent tracker IDs to add movie entries to Radarr without uploading")
        parser.add_argument('-sonarr-add', '--sonarr-add', action='store_true', required=False, help="Use queue/qBittorrent tracker IDs to add TV series entries to Sonarr without uploading")
