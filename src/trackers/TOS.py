...
class TOS(UNIT3D):
    ...
    async def upload(self, meta, disctype):
        ...
        if meta['isdir']:
            ...
            tracker_url = self.config['TRACKERS']['TOS'].get('announce_url', "https://fake.tracker").strip()
            if meta['is_disc']:
                include = []
                exclude = []
            else:
        ...
        upload_files: dict[str, tuple[str, bytes, str]] = {'torrent': ('torrent.torrent', torrent_bytes, 'application/x-bittorrent')}
            ...
            for attempt in range(max_retries):
                try:
                    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
                        response = await client.post(url=self.upload_url, files=upload_files, data=data, headers=headers, params=params)
                        ...
                        try:
                            ...
                            if error_data.get('success') is False:
                                ...
                                if 'info_hash' in data_errors:
                                    ...
                                    if 'déjà utilisée' in error_msg or 'already been taken' in error_msg:
                                        is_dupe = True
                        except Exception:
                            pass
                    ...
                    if is_dupe:
                        try:
                            ...
                            if torrent_id and download_url:
                                ...
                                console.print(f'[yellow]{self.tracker}: Torrent already exists (dupe detected)[/yellow]')
                                meta['tracker_status'][self.tracker]['status_message'] = 'Skipped: Torrent already exists on tracker'
