...
class BJS:
    ...
    async def load_localized_data(self, meta: dict[str, Any]) -> None:
        ...
        if not main_ptbr_data:
            main_ptbr_data = await get_tmdb_localized_data(meta, data_type="main", language="pt-BR", append_to_response="credits,videos,content_ratings")
        ...
        if self.config["DEFAULT"]["episode_overview"] and meta["category"] == "TV" and not meta.get("tv_pack"):
            ...
            if not episode_ptbr_data:
                episode_ptbr_data = await get_tmdb_localized_data(meta, data_type="episode", language="pt-BR", append_to_response="")
    ...
    async def _fetch_torrent_content(self, task_info: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": e, "task_info": task_info}
    ...
    async def _process_ajax_responses(self, ajax_tasks: list[dict[str, Any]], params: dict[str, Any]) -> list[dict[str, str]]:
        ...
        for result in ajax_results:
            ...
            if item_name:
                found_items.append({"name": item_name, "size": str(task_info.get("size") or ""), "link": str(task_info.get("link") or "")})
    ...
    async def get_data(self, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        if not meta.get("anime"):
            ...
            if category == "TV":
                ...
                data.update(
                    {
                        # Optional
                        "network": ", ".join([p.get("name", "") for p in self.main_tmdb_data.get("networks", [])]) or "",
                        # Optional
                        "numtemporadas": self.main_tmdb_data.get("number_of_seasons", ""),
                        "datalancamento": self.get_release_date(),
                        "pais": ", ".join(country_list),  # Optional
                        # Optional
                        "diretorserie": ", ".join(set(meta.get("tmdb_directors", []) or meta.get("imdb_info", {}).get("directors", [])[:5])),
        ...
        # Internal
        is_internal_tracker = self.config["TRACKERS"][self.tracker].get("internal", False) is True
        internal_groups = self.config["TRACKERS"][self.tracker].get("internal_groups", [])

        if is_internal_tracker and meta["tag"] != "" and meta["tag"][1:] in internal_groups:
            data.update({"internalrel": 1})
