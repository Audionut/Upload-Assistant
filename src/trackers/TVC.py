...
class TVC():
    ...
    def __init__(self, config):
        ...
        self.tv_types = [
            ...
            "foreign", "kids", "movies", "news", "reality", "soaps",
    ...
    async def get_cat_id(self, genres):
        ...
        genres = genres.split(', ')
        for i in genres:
            g = i.lower().replace(",", "").strip()
            if not g:
                continue
            for idx, s in enumerate(self.tv_types):
                if g in s:
                    return self.tv_types_ids[idx]

        # returning holding bin/misc id
        return self.tv_types_ids[13]
